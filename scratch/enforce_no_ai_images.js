...
// We will create a clean SVG placeholder string for products without a photo in the CSV:
const cleanSvgPlaceholder = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="600" height="750" viewBox="0 0 600 750" fill="%23111111"><rect width="600" height="750" fill="%2318181b"/><text x="50%" y="45%" dominant-baseline="middle" text-anchor="middle" fill="%23ffffff" font-family="sans-serif" font-size="28" font-weight="900">BahaMut</text><text x="50%" y="52%" dominant-baseline="middle" text-anchor="middle" fill="%23a1a1aa" font-family="sans-serif" font-size="16" font-weight="700">PHOTOSHOOT PENDING</text><text x="50%" y="58%" dominant-baseline="middle" text-anchor="middle" fill="%2371717a" font-family="sans-serif" font-size="12">100% Woven Cotton Denim • Ambawadi Ahmedabad</text></svg>';
...
async function run() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!parentMap[parentSku]) {
      parentMap[parentSku] = {
        ...
        hasRealPhoto: (photoStatus === 'Available' && imageUrl.startsWith('http')),
        totalStock: 0,
        availableSizes: [],
        barcodes: []
      };
    ...
    if (imageUrl && !parentMap[parentSku].imageUrl && imageUrl.startsWith('http')) {
      ...
      parentMap[parentSku].hasRealPhoto = true;
    }
  ...
  masterList.forEach(item => {
    ...
    if (item.hasRealPhoto) {
      realPhotoMap[item.parentSku] = `/images/products/${slug}.jpg`;
    } else {
      realPhotoMap[item.parentSku] = cleanSvgPlaceholder;
    }
  });

  const tsProducts = masterList.map(item => {
    const slug = item.parentSku.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const imgPath = realPhotoMap[item.parentSku];
...
`;
  ...
  console.log('✅ Updated lib/products.ts with ZERO AI-generated images (Strict Real Photo & Placeholder Enforcement)!');
}
